	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != len(tt.want) {
				t.Errorf("ParseOutput() returned %d benchmarks, want %d; got=%v", len(got), len(tt.want), got)
				return
			}
			for name, wantSamples := range tt.want {
				gotSamples, ok := got[name]
				if !ok {
					t.Errorf("ParseOutput(): missing benchmark %q in result", name)
					continue
				}
				if len(gotSamples) != len(wantSamples) {
					t.Errorf("ParseOutput()[%q] got %d samples, want %d", name, len(gotSamples), len(wantSamples))
					continue
				}
				for i, want := range wantSamples {
					if math.Abs(gotSamples[i]-want) > 1e-9 {
						t.Errorf("ParseOutput()[%q][%d] = %f, want %f", name, i, gotSamples[i], want)
					}
				}
			}
		})
