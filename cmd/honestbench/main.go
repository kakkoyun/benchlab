...
func run() int {
	...
	var allFindings []honestbench.Finding
	totalFuncs := 0

	for _, f := range files {
		findings, count, ferr := honestbench.AnalyzeFile(fset, f)
		if ferr != nil {
	...
	if *jsonOut {
		if allFindings == nil {
			allFindings = []honestbench.Finding{} // encode as [] not null
		}
	...
	if !*quiet {
		counts := honestbench.CountBySeverity(allFindings)
		fmt.Printf("\n%d findings (%d high, %d medium, %d info) across %d functions\n",
			len(allFindings),
			counts[honestbench.SeverityHigh],
			counts[honestbench.SeverityMedium],
			counts[honestbench.SeverityInfo],
			totalFuncs,
		)
	}
...
func exitCode(findings []honestbench.Finding) int {
	if len(findings) > 0 {
		return 1
	}
	return 0
...
func collectFiles(root string, recursive bool) ([]string, error) {
	if root == "..." || strings.HasSuffix(root, "/...") || strings.HasSuffix(root, `\...`) {
		recursive = true
	...
	if recursive {
		walkErr := filepath.WalkDir(root, func(path string, d fs.DirEntry, walkErr error) error {
			...
			// Skip nested hidden and vendor directories, but never skip the walk root.
			if path != root && d.IsDir() && (strings.HasPrefix(d.Name(), ".") || d.Name() == "vendor") {
